...
import (
	...
	"github.com/khulnasoft/kengine-plugin-sdk-go/live"
)
...
// CheckHealth handles health checks sent from Kengine to the plugin.
...
func (d *SampleDatasource) RunStream(ctx context.Context, req *backend.RunStreamRequest, sender *backend.StreamSender) error {
	...
	// Stream data frames periodically till stream closed by Kengine.
	for {
		select {
		case <-ctx.Done():
